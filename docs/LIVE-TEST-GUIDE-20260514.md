...
|------------|------|
| **20:35** | **처음으로** 버튼 → 엔진 리셋 |
| **20:35** | `35864` 입력 후 **폼 조회** (formOpenAt = ~20:25 이므로 이미 열려있어야 함) |
...
### 폼 조회 (20:15)
- [ ] 이벤트 URL 또는 ID 입력: `35863` (또는 전체 URL `https://weverse.io/tws/notice/35863`)
- [ ] **폼 조회** 클릭
...
### 1. eventId 매핑 문제
**상황**: notice URL의 번호(`35863`)가 fanevent-v2 API의 eventId와 다를 수 있음
**확인**: 폼 조회 시 에러가 나면 로그에서 실제 API 호출 URL 확인
```
WeverseApi fetchFormSchema eventId=35863
```
**대응**: 공지 페이지 안의 "참여신청" 버튼 링크에서 실제 eventId를 확인해야 할 수 있음
- 공지 페이지를 브라우저에서 열고 참여신청 버튼의 href에서 `/events/{실제ID}` 추출
- 추출한 ID를 앱에 직접 입력
...
| 폼 조회 계속 실패 | 브라우저에서 공지 페이지 열어 참여신청 링크의 실제 eventId 확인 |
