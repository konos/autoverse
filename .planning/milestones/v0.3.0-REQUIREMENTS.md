# Requirements Archive: v0.3.0 로그인 방식 선택 (API / 브라우저)
...
| R016 | core-capability | validated | v0.3.0/Phase 06 | none | Phase 06: `settings.json` 영속 + `AUTOVERSE_LOGIN_MODE` 잠금, 저장 실패 시 편도(success-only) 갱신. 06-UAT Test 1·4·7 실행 확인. |
| R017 | core-capability | out-of-scope | none | none | 2026-09-07 v0.3.0 종료 시 out-of-scope 로 이동. "브라우저 엔진 없이 순수 HTTP 인증"은 reCAPTCHA Enterprise 관문 때문에 성립하지 않으며, 우회는 R013(영구 제외)이다. 계약 자체는 05-01 HAR 실측으로 확정됐고(`by-credentials` 단독 + 캡차 토큰 필수), 동작하는 유일한 경로는 헤드리스 BrowserWindow 다. |
| R018 | core-capability | out-of-scope | none | none | 2026-09-07 v0.3.0 종료 시 blocked → out-of-scope. HAR 실측상 OTP 단계가 실제 로그인 흐름에 존재하지 않는다(호출 0건) — 미구현이 아니라 대상 자체가 없다. Phase 06 D-02 가 관련 코드를 전량 삭제했다. |
...
| R020 | failure-visibility | validated | v0.3.0/Phase 06 | none | Phase 06: `mapLoginFailure()` 6개 실패 신호 exhaustive switch 매핑 + `buildFailureResult()` 단일 마스킹 관문. 06-UAT Test 8 실행 확인. |
| R021 | failure-visibility | validated | v0.3.0/Phase 06 | none | Phase 06: 네이티브 `<dialog>.showModal()` 차단형 고지 모달 — 확인/취소 경로 물리적 분리. 06-UAT Test 1·2·3 실행 확인. |
| R022 | safety-guard | validated | v0.3.0/Phase 07 | none | Phase 07: `token-expiry.ts` 순수 판정 → `arm()` 즉시 경고 → 대기 화면 인라인 배너 → 대기 중 재로그인 → `shouldRecheckTokenExpiry()` 재판정 seam. 대기 이전 토큰 캡처 결함(D-13) 폐쇄로 재로그인이 실제 POST 에 반영된다. 07-UAT Test 1·3·4 실행 확인. |
| R023 | core-capability | validated | v0.3.0/Phase 07 | none | Phase 07: safeStorage 저장 + `credentials.enc` 4상태 계약 + main 프로세스 이메일 불일치 최종 게이트 + in-flight 가드. 평문 비밀번호는 IPC·렌더러·React state 어디에도 없다(D-01, 타입 수준 강제). 07-UAT Test 2·5·6 실행 확인. |

## Coverage Summary

*v0.3.0 종료 시점 (2026-09-07) 기준.*

- Active requirements: 3 (R006 로그 패널, R008 크로스플랫폼 빌드, R010 로그 마스킹 — 전부 M001-ksbtje 소유. M001 종료 시 표가 갱신되지 않아 `active`/`unmapped` 로 남았고, PROJECT.md 서술상으로는 구현됐다. v0.3.0 스코프 밖이므로 이번 종료에서 판정하지 않고 다음 마일스톤으로 이월한다.)
- Validated: 13 (R001, R002, R003, R004, R005, R007, R009 — M001 / R016, R019, R020, R021, R022, R023 — v0.3.0)
- Out of scope: 7 (R011, R012, R013, R014, R015 — 기존 / **R017, R018 — 2026-09-07 v0.3.0 종료 시 이동**)
- Blocked: 0 (R018 은 blocked → out-of-scope 로 재분류)
- Unmapped active requirements: 3 (위 M001 이월 3건)

**R017/R018 재분류 근거:** 두 요구사항 모두 "아직 못 했다"가 아니라 "대상이 존재하지 않거나 구조적으로 불가능하다"로 판명됐다. R017 의 "브라우저 엔진 없이 순수 HTTP 인증"은 reCAPTCHA Enterprise 관문이 사람/브라우저 개입을 요구해 성립하지 않으며, 우회는 R013 으로 영구 제외다. R018 의 이메일 OTP 단계는 2026-08-25 HAR 실측에서 호출 0건 — 실제 로그인 흐름에 존재하지 않는다. 상황이 바뀌면(공식 API 공개 등) 새 요구사항으로 다시 세우는 편이 정확하다.
